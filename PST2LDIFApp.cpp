// CPST2LDIFApp.cpp: implementation of the CPST2LDIFApp class.
//
//////////////////////////////////////////////////////////////////////
#include "stdafx.h"
...
CPST2LDIFApp::CPST2LDIFApp() :
m_pstProc(new CPSTProc()), m_xmlProc(new CXMLDirProc()), m_verbose(false), m_printFormsOnly(false)
{
}
...
_TCHAR *CPST2LDIFApp::PrefixWithCurDir(_TCHAR *fileName, _TCHAR *fileNameFull)
{
...
int CPST2LDIFApp::ProcessCommandLine(int argc, _TCHAR *argv[])
{
...
void CPST2LDIFApp::ShowUsage()
{
	_TCHAR usage[256];
	HINSTANCE hInst = ::GetModuleHandle(NULL);

// Usage: PST2LDIF [OPTION]...
...
CLdifEntry* CPST2LDIFApp::getNextLdifPersonEntry()
{
	return m_xmlProc->getNextLdifPersonEntry();
}

CLdifEntry* CPST2LDIFApp::getNextLdifGroupEntry()
{
	return m_xmlProc->getNextLdifGroupEntry();
}

CLdifEntryFilter* CPST2LDIFApp::getNextLdifEntryFilter(bool person)
{
	return m_xmlProc->getNextLdifEntryFilter(person);
}

int CPST2LDIFApp::getCurrentLdifEntryFilterIndex()
{
	return m_xmlProc->getCurrentLdifEntryFilterIndex();
}

void CPST2LDIFApp::setCurrentLdifEntryFilterIndex(int index)
{
	m_xmlProc->setCurrentLdifEntryFilterIndex(index);
}

void CPST2LDIFApp::resetEntryFilterIndex()
{
	if (m_xmlProc != NULL)
		m_xmlProc->resetEntryFilterIndex();
}

void CPST2LDIFApp::resetEntryListIndex()
{
	if (m_xmlProc != NULL)
		m_xmlProc->resetEntryListIndex();
}

void CPST2LDIFApp::resetTransformerListIndex()
{
	m_xmlProc->resetTransformerListIndex();
}

CLdifEntryTransformer* CPST2LDIFApp::getNextLdifEntryTransformer()
{
	return m_xmlProc->getNextLdifEntryTransformer();
}

CLdifEntryTransformer* CPST2LDIFApp::getNextSMTPEntryTransformer()
{
	return m_xmlProc->getNextSMTPEntryTransformer();
}

const _TCHAR* CPST2LDIFApp::getConfigSettingValue(const _TCHAR* settingName)
{
	return m_xmlProc->getConfigSettingValue(settingName);
}

void CPST2LDIFApp::insertLdifEntry(const _TCHAR *attribute, const _TCHAR *value)
{
	m_xmlProc->insertLdifEntry(attribute, value);
}

CSmtpAddr* CPST2LDIFApp::getSmtpAddrInfo()
{
	return m_xmlProc->getSmtpAddrInfo();
}

CPersonGroupEntry *CPST2LDIFApp::getPersonGroupEntry(bool isPerson)
{
	return m_xmlProc->getPersonGroupEntry(isPerson);
}

bool CPST2LDIFApp::matchesRegExp(char *regExpPattern, char* matchString)
{
	return m_xmlProc->matchesRegExp(regExpPattern, matchString);
}

int CPST2LDIFApp::ExtractSchemaFile(_TCHAR* tempDir)
{
	_TCHAR usage[256];
	HINSTANCE hInst = ::GetModuleHandle(NULL);
	_TCHAR current_method[] = _TEXT("CPST2LDIFApp::ExtractSchemaFile");
	if (IsVerbose()){
...
int CPST2LDIFApp::StartMigration(_TCHAR* exec)
{
	_TCHAR usage[256];
	HINSTANCE hInst = ::GetModuleHandle(NULL);
	_TCHAR current_method[] = _TEXT("CPST2LDIFApp::StartMigration");
	if (IsVerbose()){
		::LoadString(hInst, IDS_ENTER_METHOD, usage, 256);
		::_tprintf(_TEXT("%s %s\n"), usage, current_method);
	}
...
int wmain(int argc, _TCHAR* argv[])
{
	...
	CPST2LDIFApp *theApp = new CPST2LDIFApp();
